...
lasReader = lasFileReader('F:/.../data/Plot_Trunk_Slice.las');
...
for i = 1:length(treeLabels)
    ...
    if rangeX < 0.3 && convexArea > (pi*(0.3/2)^2)/3
...
for i = 1:length(treeLabels2)
    ...
    [centersPointCloud, radii_m] = Hough_Transform(currentTreePoints2, 0.01, 8, 50, 0.93);
...
writematrix(output_LHD, 'F:/.../data/output_LHD.csv');
